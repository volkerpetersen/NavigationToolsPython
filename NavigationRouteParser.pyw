...
class Navigation_Route_Parser(wx.Frame):
	...
	def __init__(self, cwdPath, gpx, sql, routes):
		global left, right, log
		...
		left.path = wx.TextCtrl(left, -1, self.routes, pos=(5, 60), size=(blength + 30, bheight + 10),
...
#--------------------------------------------------------------------------------------------
	def onComputeDistances(self, event):
		global filename, left, right, log

		try:
			file = os.path.join(self.pathGPX, filename)
...
#--------------------------------------------------------------------------------------------
	def onParseSQLRouteFile(self, event):
		global right, log, filename, sql

		log = parseSQLRouteFile(self.pathSQL, sql, filename)
		right.log.SetValue(log + "\n\n")
...
#--------------------------------------------------------------------------------------------
	def onParseGPXRouteFile(self, event):
		...
		try:
			inputfile = open(os.path.join(self.pathSQL, filename), "r")
			xml = inputfile.read()
			inputfile.close

		except:
			print ("Error opening file: %s" %self.pathSQL)
		...
		if values in xml:
		   ...
		   wps = xml.split("),")
			...
			msg = msg + " in the MySQL route file " + self.pathSQL +"\n\n"
			log_msg = msg + log_msg
			log = log_msg + "\n\n" + log
			right.log.SetValue(log)
			return
		...
		OutputFile = open(os.path.join(self.pathGPX, filename2), "w")
		...
		for wp in wps:
			...
			if len(wp_data) > 3:
				...
				if (wp_data[2].startswith('WP0')):
					...
					output += '<sym>'+wp_data[5].replace("'", "")+'</sym>\n'
				else:
					output += '<sym>empty</sym>\n'
		...
		msg = msg + " from the MySQL route file " + os.path.join(self.pathSQL, filename) + " to a "
		msg = msg + os.path.join(self.pathGPX, filename2) + " file.\n\n"
		log_msg = msg + log_msg
		log = log_msg + "\n\n" + log
		right.log.SetValue(log)
		return
...
INSERT INTO `Table_Name` (`id`, `from`, `to`, `lat`, `lon`, `type`, `image`, `notes`) VALUES\n"""
		...
		try:
			...
			print ("Error opening file: %s" %os.path.join(self.pathGPX, filename))

		soup = BeautifulSoup(xml, "xml")
		wps = soup.find_all('trkpt')

		path2 = os.path.join(self.pathSQL, filename)
		path2 = path2.replace('.spot', '.sql')
...
#--------------------------------------------------------------------------------------------
	def onUploadMySQLFile(self, event):
		global filename, right, log

		print(os.path.join(self.pathSQL, "ToernDirectoryTable.sql"))
		
		log_msg = ""
		msg1 = uploadMySQLfile(os.path.join(self.pathSQL, "ToernDirectoryTable.sql"))
		if not msg1:
		...
		msg2 = uploadMySQLfile(os.path.join(self.pathSQL, filename))
		if not msg2:
...
def calc_distance(latFrom, lonFrom, latTo, lonTo):
	...
	RADIUS = 6378.137*KM_NM		# Earth RADIUS (in km) as per Google Maps converted to nm 
		...
		if (ctr > 0):
			...
			if lat == first_lat and lon == first_lon:
				...
				if 'anchorage' in symbol:
					...
					route = 'route'
			output += "(" + str(ctr)+", '" + old_name +"', '" + name +"', " + "'" + str(lat) + "', '" +str(lon) + "', '"
			output += route + "', '', " + notes + "),\n"
...
if __name__ == "__main__":
	...
	frame = Navigation_Route_Parser(cwd, gpxPath, sqlPath, openCPNroutes)
	frame.Show()
	app.MainLoop()
